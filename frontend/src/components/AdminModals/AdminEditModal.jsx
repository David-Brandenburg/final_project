...
const AdminEditModal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = `${URL}/${
        adminTab === "accounts" ? "accounts/updateAccountInfo" : "games"
      }/${adminEditModal}`;
  ...
  return (
    <div className="admin-edit-modal" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="modal-content-bottom">
        <form
          className="modal-content modal-content-left"
          onSubmit={handleSubmit}>
          {data && adminTab === "accounts" && (
            <div className="form-row">
              <div className="edit-row">
                <label htmlFor="email">
                  {language === "en" ? "Edit E-Mail" : "E-Mail ändern"}:
                </label>
                <input
                  type="email"
                  name="email"
              ...
              <div className="edit-row">
                <label htmlFor="benutzername">
                  {language === "en" ? "Edit Username" : "Benutzername ändern"}:
                </label>
                <input
                  type="text"
                  name="benutzername"
                  id="benutzername"
                  defaultValue={data.benutzername}
                  onChange={handleChange}
                />
              </div>
              <div className="edit-row">
                <label htmlFor="vorname">
                  {language === "en" ? "Edit Firstname" : "Vorname ändern"}:
                </label>
                <input
                  type="text"
                  name="vorname"
                  id="vorname"
                  defaultValue={data.vorname}
                  onChange={handleChange}
                />
              </div>
              <div className="edit-row">
                <label htmlFor="nachname">
                  {language === "en" ? "Edit Lastname" : "Nachname ändern"}:
                </label>
                <input
                  type="text"
                  name="nachname"
                  id="nachname"
                  defaultValue={data.nachname}
                  onChange={handleChange}
                />
              </div>
              <div className="edit-row checkbox-row">
                <label htmlFor="isAdmin">
                  {language === "en" ? "User is admin?" : "Nutzer ist Admin?"}
                </label>
                <input
                  type="checkbox"
                  name="isAdmin"
                  id="isAdmin"
                  checked={objectToEdit.isAdmin || false}
                  onChange={handleChange}
                />
              </div>
            </div>
            ...
            <>
              <>
                <div className="user-profilepic-wrapper">
                  <img
                    src={data.profilePic ? data.profilePic : defaultPic}
                    alt=""
                  />
                </div>
              </>
              <hr className="horizontal-hr" />
              <div className="edit-row button-row">
                <button className="btn-cancel" onClick={resetAccountProfilePic}>
                  Delete Image
                </button>
              </div>
            </>
