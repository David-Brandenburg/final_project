		...
		<nav>
			<div className='nav-wrapper' onClick={handleCloseAll}>
				<div className='nav-left'>
					...
					{!openSearch && <div className='item-wrapper'>
						<NavLink to='/games' className='nav-link' title='navlink'>Spiele</NavLink>
				...
				{openSearch && <div className='nav-mid'>
					<label htmlFor="search"><i className='bi bi-search'></i></label>
					<input type="text" className='nav-search' name="search" id="search" autoFocus onClick={((e) => e.stopPropagation())}/>
				</div>}
				<div className="nav-right">
					...
					<div className='cart-wrapper' onClick={handleOpenCart}>
						...
						<small className={`cart-count ${cIL > 0 ? 'cart-items' : 'cart-empty'}`} style={{color: cIL > 0 ? '#fff' : 'unset', background: cIL > 0 ? '#f85525' : 'unset'}}>{cIL}</small> {/* Cart Count */}
						{openCart && <div className='cart-modal' onClick={((e) => e.stopPropagation())}>
							<p>Cart Modal</p>
						</div>}
					</div>
					<div className='screenmode-wrapper' onClick={handleScreenMode}>
						{screenMode === 'light' ? <i className="bi bi-moon"></i> : <i className="bi bi-brightness-high"></i>}
					</div>
				</div>
			</div>
		</nav>
