      ...
      {games &&
        <div className="dynamic-gameslist-wrapper">
          {games.slice(0, 6).map((game, index) => (
            <div key={`game-card`+index} className="game-card" onMouseEnter={() => handleMouseEnter(game)} onMouseLeave={handleMouseLeave}>
              {hoveredGame === game && <GamesModal game={game} />}
              <div className="game-card-thumbnail-wrapper">
                <img src={game.thumbnail} alt={game.title} className="game-card-thumbnail"/>
              </div>
              <div className="game-card-content">
                ...
                <div className="game-card-pricetags">
									{game.discount > 0 && <p className="game-discount-tag">-{game.discount}%</p>}
									<div className="game-pricetags">
										{game.discount > 0 && <small className="game-pricetag-discount">{game.price} €</small>}
										<p className="game-card-price">{game.discount > 0 ? (Math.floor((game.price - (game.price * game.discount) / 100) * 100) / 100) : game.price} €</p>
									</div>
								</div>
              </div>
            </div>
